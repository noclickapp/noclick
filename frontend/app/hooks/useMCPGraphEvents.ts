/**
 * Hook for handling graph changes from the MCP endpoint.
 *
 * Listens for 'mcp:graph_event' socket events emitted when external MCP clients
...
interface MCPGraphEvent {
    workflow_id: string;
    event_type: string;
    data: Record<string, any>;
}

interface UseMCPGraphEventsParams {
    workflowId?: string;
    setNodes: React.Dispatch<React.SetStateAction<Node[]>>;
    setEdges: React.Dispatch<React.SetStateAction<Edge[]>>;
}

export function useMCPGraphEvents({
    workflowId,
    setNodes,
    setEdges,
}: UseMCPGraphEventsParams) {
    ...
    const handleEvent = useCallback((event: MCPGraphEvent) => {
        ...
        switch (event_type) {
            case 'node_start': {
                ...
                if (!nodeData) break;
                if (!nodeData.id) {
                    console.error('[useMCPGraphEvents] node_start missing id:', nodeData);
                    break;
                }
