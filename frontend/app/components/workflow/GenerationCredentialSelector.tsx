/**
 * GenerationCredentialSelector - Credential selection UI for workflow generation flow.
 * Provides a simplified UI for selecting/connecting credentials during the "Setup Required" phase.
...
interface GenerationCredentialSelectorProps {
...
/** Agent nodes have a config-sensitive credential form (harness + sub-model pick
 * the fields, incl. subscription OAuth) — delegate to NodeCredentials, which
 * routes nodeType='agent' to AgentCredentialsForm. The provider-registry lookup
 * below can't resolve agent_* credential types. */
function AgentGenerationCredential({
    input,
    onCredentialSelect,
}: Pick<GenerationCredentialSelectorProps, 'input' | 'onCredentialSelect'>) {
    const [ids, setIds] = useState<Record<string, string>>(input.credentialIds || {});
    return (
        <NodeCredentials
            nodeType="agent"
            nodeData={{ config: input.nodeConfig || {} }}
            credentialIds={ids}
            onChange={(newIds) => {
                setIds(newIds);
                const firstId = Object.values(newIds).find(id => id && id.trim());
                onCredentialSelect(firstId || '');
            }}
            compact
        />
    );
...
export function GenerationCredentialSelector(props: GenerationCredentialSelectorProps) {
    if (props.input.nodeType === 'agent') {
        return <AgentGenerationCredential input={props.input} onCredentialSelect={props.onCredentialSelect} />;
    }
    return <GenerationCredentialSelectorInner {...props} />;
}

function GenerationCredentialSelectorInner({
    input,
    onCredentialSelect,
    selectedCredentialId,
    nodeType,
    credentialVariables,
}: GenerationCredentialSelectorProps) {
    ...
    const providerConfig = useMemo(() => {
        ...
        if (input.acceptedCredentialTypes && input.acceptedCredentialTypes.length > 0) {
            ...
            if (config) {
                console.debug(`[GenerationCredentialSelector] Found provider via acceptedCredentialTypes: ${input.acceptedCredentialTypes[0]} → ${config.name}`);
                return config;
            }
        }
        ...
        if (input.credentialType) {
            ...
            if (config) {
                console.debug(`[GenerationCredentialSelector] Found provider via credentialType: ${input.credentialType} → ${config.name}`);
                return config;
            }
        }
        ...
        if (input.credentialType) {
            ...
            if (config) {
                console.debug(`[GenerationCredentialSelector] Found provider via normalized key: ${input.credentialType} → ${normalized} → ${config.name}`);
                return config;
            }
        }

        console.warn(`[GenerationCredentialSelector] Could not find provider config for credentialType="${input.credentialType}", acceptedCredentialTypes=${JSON.stringify(input.acceptedCredentialTypes)}`);
        return undefined;
    ...
    const handleCreateCredential = useCallback(async () => {
        ...
        try {
            ...
            console.error('[GenerationCredentialSelector] Error creating credential:', err);
    ...
    if (!providerConfig && nonOAuthCredentials.length === 0) {
        console.warn(`[GenerationCredentialSelector] Unknown provider: "${input.credentialType}" and no schema found`);
        return (
    ...
    return (
        <div className="space-y-2">
            ...
            <UpgradePopup
                ...
                onOpenChange={(open) => { if (!open) { setPlanLimitError(null); clearError(); } }}
