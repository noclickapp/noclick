...
const SURFACE = 'rounded-xl border border-border bg-foreground/[0.02] dark:border-border/40 dark:bg-gradient-to-b dark:from-card/30 dark:to-card/10 relative overflow-hidden';
...
function MorphAvatar({ delay = 0 }: { delay?: number }) {
    return <div className="w-6 h-6 rounded-full bg-muted-foreground/25 dark:bg-muted/50" style={{ animation: `ifa-avatar-pulse ${INNER_CYCLE}ms ease-in-out infinite`, animationDelay: `${delay}ms` }} />;
}
