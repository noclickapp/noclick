// Typeform OAuth callback route.
// Handles the redirect from Typeform after user grants permission.
...
export async function loader({
    ...
    if (error) {
        console.error(
            '[typeform.callback] OAuth error:',
            error,
            errorDescription
        );
        return {
            success: false,
            error: errorDescription || `Typeform OAuth error: ${error}`,
        };
    }

    if (!code || !stateB64) {
        console.error('[typeform.callback] Missing code or state');
        return {
            success: false,
    ...
    try {
        ...
        // Use the same fixed redirect_uri that was sent during authorization
        const redirectUri =
            process.env.TYPEFORM_REDIRECT_URI?.trim() ||
        ...
        console.error('[typeform.callback] Failed to decode state');
        return {
            success: false,
...
export default function TypeformOAuthCallback() {
