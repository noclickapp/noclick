// Canva OAuth callback route with PKCE support.
// Handles the redirect from Canva after user grants permission.
...
export async function loader({
    ...
    if (error) {
        console.error('[canva.callback] OAuth error:', error, errorDescription);
        return {
            success: false,
            error: errorDescription || `Canva OAuth error: ${error}`,
        };
    }
    ...
    try {
        ...
        console.error('[canva.callback] Failed to decode state');
        return {
            success: false,
    ...
    useEffect(() => {
        if (window.opener) {
            ...
            try {
                ...
                console.error('[CanvaCallback] Failed to notify opener');
            }
