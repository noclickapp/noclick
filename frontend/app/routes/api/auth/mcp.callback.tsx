...
// Handles the redirect from MCP server's OAuth provider after user grants permission.
...
export async function loader({
    ...
    if (error) {
        console.error('[mcp.callback] OAuth error:', error, errorDescription);
        return {
            success: false,
            error,
            error_description: errorDescription || `MCP OAuth error: ${error}`,
        };
    }

    if (!code) {
        console.error('[mcp.callback] Missing authorization code');
        return {
            success: false,
            error: 'missing_code',
