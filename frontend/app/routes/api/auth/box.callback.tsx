// Box OAuth callback route.
// Handles the redirect from Box after the user grants permission.
...
export async function loader({
    ...
    if (error) {
        console.error('[box.callback] OAuth error:', error, errorDescription);
        return {
            success: false,
            error: errorDescription || `Box OAuth error: ${error}`,
        };
    }

    if (!code || !stateB64) {
        console.error('[box.callback] Missing code or state');
        return {
            success: false,
    ...
    try {
        ...
        console.error('[box.callback] Failed to decode state:', e);
        return {
            success: false,
...
export default function BoxOAuthCallback() {
