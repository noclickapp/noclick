...
// Handles the redirect from Atlassian after user grants permission.
...
import { decodeAtlassianOAuthState } from '~/utils/atlassianOAuthState.server';
...
export async function loader({
    ...
    if (error) {
        console.error(
            '[atlassian.callback] OAuth error:',
            error,
            errorDescription
        );
        return {
            success: false,
            error: errorDescription || `Atlassian OAuth error: ${error}`,
        };
    }

    if (!code || !stateB64) {
        console.error('[atlassian.callback] Missing code or state');
        return {
            success: false,
    ...
    // Decode state
    try {
        const state = decodeAtlassianOAuthState(stateB64);
        ...
        return {
            ...
            scopes: state.scopes,
            jiraSite: state.jiraSite,
            appOrigin: state.appOrigin,
        };
    } catch (e) {
        console.error('[atlassian.callback] Failed to decode state:', e);
        return {
            success: false,
...
export default function AtlassianOAuthCallback() {
