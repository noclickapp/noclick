// Klaviyo OAuth callback route with PKCE support.
// Handles the redirect from Klaviyo after user grants permission.
// Passes the authorization code and PKCE verifier back to the opener window via postMessage.
...
export async function loader({
    ...
    if (error) {
        console.error(
            '[klaviyo.callback] OAuth error:',
            error,
            errorDescription
        );
        // The user clicking "Deny" on Klaviyo's consent screen comes back as
        // access_denied — surface clear guidance to connect again rather than a
        // raw OAuth error code (Klaviyo app-review requirement).
        const denied = error === 'access_denied';
        return {
            success: false,
            error: denied
                ? 'You denied the permissions NoClick needs to connect to Klaviyo. To use the integration, connect again and click Allow.'
                : errorDescription || `Klaviyo OAuth error: ${error}`,
        };
    }

    if (!code || !stateB64) {
        console.error('[klaviyo.callback] Missing code or state');
        return {
            success: false,
    ...
    // Decode state (contains PKCE verifier)
    try {
        ...
        return {
            ...
            codeVerifier: state.codeVerifier, // PKCE verifier for token exchange
            credentialName: state.credentialName,
            scopes: state.scopes,
        };
    } catch (e) {
        console.error('[klaviyo.callback] Failed to decode state:', e);
        return {
            success: false,
...
export default function KlaviyoOAuthCallback() {
