// Airtable OAuth callback route with PKCE support.
// Handles the redirect from Airtable after user grants permission.
...
export async function loader({
    ...
    if (error) {
        console.error(
            '[airtable.callback] OAuth error:',
            error,
            errorDescription
        );
        return {
            success: false,
            error: errorDescription || `Airtable OAuth error: ${error}`,
        };
    }

    if (!code || !stateB64) {
        console.error('[airtable.callback] Missing code or state');
        return {
            success: false,
    ...
    try {
        ...
        console.error('[airtable.callback] Failed to decode state:', e);
        return {
            success: false,
...
export default function AirtableOAuthCallback() {
