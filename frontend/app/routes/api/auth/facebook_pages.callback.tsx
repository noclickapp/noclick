...
export async function loader({
    ...
    if (error) {
        console.error(
            '[facebook_pages.callback] OAuth error:',
            error,
            errorDescription
        );
        return {
    ...
    if (!code || !stateB64) {
        console.error('[facebook_pages.callback] Missing code or state');
        return {
            success: false,
    ...
    try {
        ...
        return {
            success: true,
            code,
            redirectUri: process.env.FACEBOOK_PAGES_REDIRECT_URI,
            scopes: state.scopes,
        };
    } catch (e) {
        console.error('[facebook_pages.callback] Failed to decode state:', e);
        return {
            success: false,
    ...
    useEffect(() => {
        ...
        if (window.opener) {
            window.opener.postMessage(
                {
                    type: 'facebook_pages-oauth-callback',
                    ...data,
                },
