// Sentry OAuth callback route.
// Handles the redirect from Sentry after the user grants permission.
...
export async function loader({
    ...
    if (error) {
        console.error(
            '[sentry.callback] OAuth error:',
            error,
            errorDescription
        );
        return {
            success: false,
            error: errorDescription || `Sentry OAuth error: ${error}`,
        };
    }
    if (!code || !stateB64) {
        console.error('[sentry.callback] Missing code or state');
        return {
            success: false,
    ...
    try {
        ...
        console.error('[sentry.callback] Failed to decode state:', e);
        return { success: false, error: 'Invalid state parameter' };
    }
}

export default function SentryOAuthCallback() {
    ...
    useEffect(() => {
        if (window.opener) {
            window.opener.postMessage(
                { type: 'sentry-oauth-callback', ...data },
