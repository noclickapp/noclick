// Cal.com OAuth callback route.
// Handles the redirect from Cal.com after user grants permission.
...
export async function loader({
    ...
    if (error) {
        console.error(
            '[calcom.callback] OAuth error:',
            error,
            errorDescription
        );
        return {
            success: false,
            error: errorDescription || `Cal.com OAuth error: ${error}`,
        };
    }

    if (!code || !stateB64) {
        console.error('[calcom.callback] Missing code or state');
        return {
            success: false,
    ...
    try {
        ...
        console.error('[calcom.callback] Failed to decode state:', e);
