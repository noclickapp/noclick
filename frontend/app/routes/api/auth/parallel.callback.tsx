// Parallel OAuth callback route.
// Handles the redirect from Parallel after user grants permission.
// Passes the authorization code and PKCE verifier back to the opener window
// so the hook can complete the token exchange via backend socket event.
...
export async function loader({
    ...
    if (error) {
        console.error(
            '[parallel.callback] OAuth error:',
            error,
            errorDescription
        );
        return {
            success: false,
            error: errorDescription || `Parallel OAuth error: ${error}`,
        };
    }

    if (!code || !stateB64) {
        console.error('[parallel.callback] Missing code or state');
        return {
            success: false,
    ...
    try {
        ...
        console.error('[parallel.callback] Failed to decode state:', e);
        return { success: false, error: 'Invalid state parameter' };
    }
}

export default function ParallelOAuthCallback() {
    ...
    useEffect(() => {
        if (window.opener) {
            window.opener.postMessage(
                { type: 'parallel-oauth-callback', ...data },
