...
// Handles the redirect from Salesforce after user grants permission.
...
interface LoaderData {
    ...
    scopes?: string[];
    isSandbox?: boolean;
    codeVerifier?: string; // PKCE: Required for token exchange
    error?: string;
}
...
export async function loader({
    ...
    if (error) {
        console.error(
            '[salesforce.callback] OAuth error:',
            error,
            errorDescription
        );
        return {
            success: false,
            error: errorDescription || `Salesforce OAuth error: ${error}`,
        };
    }

    if (!code || !stateB64) {
        console.error('[salesforce.callback] Missing code or state');
        return {
            success: false,
    ...
    try {
        ...
        return {
            ...
            codeVerifier: state.codeVerifier, // PKCE: Pass verifier for token exchange
        };
    } catch (e) {
        console.error('[salesforce.callback] Failed to decode state:', e);
        return {
            success: false,
