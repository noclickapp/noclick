// ClickUp OAuth callback route.
// Handles the redirect from ClickUp after the user grants permission.
...
export async function loader({
    ...
    if (error) {
        console.error(
            '[clickup.callback] OAuth error:',
            error,
            errorDescription
        );
        return {
            success: false,
            error: errorDescription || `ClickUp OAuth error: ${error}`,
        };
    }

    if (!code || !stateB64) {
        console.error('[clickup.callback] Missing code or state');
        return {
            success: false,
    ...
    try {
        ...
        console.error('[clickup.callback] Failed to decode state:', e);
        return {
            success: false,
...
export default function ClickUpOAuthCallback() {
