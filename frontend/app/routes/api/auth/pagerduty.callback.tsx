// PagerDuty OAuth callback route.
// Handles the redirect from PagerDuty after the user grants permission.
...
export async function loader({
    ...
    if (error) {
        console.error(
            '[pagerduty.callback] OAuth error:',
            error,
            errorDescription
        );
        return {
            success: false,
            error: errorDescription || `PagerDuty OAuth error: ${error}`,
        };
    }

    if (!code || !stateB64) {
        console.error('[pagerduty.callback] Missing code or state');
        return {
            success: false,
    ...
    try {
        ...
        return {
            success: true,
            code,
            redirectUri: process.env.PAGERDUTY_REDIRECT_URI,
            scopes: state.scopes,
        };
    } catch (e) {
        console.error('[pagerduty.callback] Failed to decode state:', e);
        return {
            success: false,
