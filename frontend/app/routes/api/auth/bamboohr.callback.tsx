// BambooHR OAuth callback route.
// Handles the redirect from BambooHR after the user grants permission, and
// passes the authorization code (plus the subdomain from state) back to the
...
export async function loader({
    ...
    if (error) {
        console.error(
            '[bamboohr.callback] OAuth error:',
            error,
            errorDescription
        );
        return {
            success: false,
            error: errorDescription || `BambooHR OAuth error: ${error}`,
        };
    }

    if (!code || !stateB64) {
        console.error('[bamboohr.callback] Missing code or state');
        return {
            success: false,
    ...
    try {
        ...
        return {
            success: true,
            code,
            redirectUri: process.env.BAMBOOHR_REDIRECT_URI,
            subdomain: state.subdomain,
            scopes: state.scopes,
        };
    } catch (e) {
        console.error('[bamboohr.callback] Failed to decode state:', e);
        return { success: false, error: 'Invalid state parameter' };
    }
}

export default function BambooHROAuthCallback() {
    ...
    useEffect(() => {
        if (window.opener) {
            window.opener.postMessage(
                { type: 'bamboohr-oauth-callback', ...data },
