...
// Handles the redirect from Stripe after the user grants permission.
...
export async function loader({
    ...
    if (error) {
        console.error(
            '[stripe.callback] OAuth error:',
            error,
            errorDescription
        );
        return {
            success: false,
            error: errorDescription || `Stripe OAuth error: ${error}`,
        };
    }

    if (!code || !stateB64) {
        console.error('[stripe.callback] Missing code or state');
        return {
            success: false,
    ...
    try {
        ...
        console.error('[stripe.callback] Failed to decode state:', e);
        return {
            success: false,
