// Cloudflare OAuth callback route.
// Handles the redirect from Cloudflare after user grants permission.
...
export async function loader({
    ...
    if (error) {
        console.error(
            '[cloudflare.callback] OAuth error:',
            error,
            errorDescription
        );
        return {
            success: false,
            error: errorDescription || `Cloudflare OAuth error: ${error}`,
        };
    }

    if (!code || !stateB64) {
        console.error('[cloudflare.callback] Missing code or state');
        return {
            success: false,
    ...
    try {
        ...
        return {
            success: true,
            code,
            redirectUri: process.env.CLOUDFLARE_REDIRECT_URI,
            scopes: state.scopes,
        };
    } catch (e) {
        console.error('[cloudflare.callback] Failed to decode state:', e);
    ...
    useEffect(() => {
        if (window.opener) {
            window.opener.postMessage(
                { type: 'cloudflare-oauth-callback', ...data },
