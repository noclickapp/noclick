...
// Handles the redirect from Mailchimp after user grants permission.
...
export async function loader({
    ...
    if (error) {
        console.error(
            '[mailchimp.callback] OAuth error:',
            error,
            errorDescription
        );
        return {
            success: false,
            error: errorDescription || `Mailchimp OAuth error: ${error}`,
        };
    }

    if (!code || !stateB64) {
        console.error('[mailchimp.callback] Missing code or state');
        return {
            success: false,
    ...
    try {
        ...
        console.error('[mailchimp.callback] Failed to decode state:', e);
        return {
            success: false,
...
export default function MailchimpOAuthCallback() {
