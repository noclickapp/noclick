...
export async function loader({
    ...
    if (error) {
        console.error(
            '[fathom.callback] OAuth error:',
            error,
            errorDescription
        );
        return {
            success: false,
            error: errorDescription || `Fathom OAuth error: ${error}`,
        };
    }

    if (!code || !stateB64) {
        console.error('[fathom.callback] Missing code or state');
        return {
            success: false,
    ...
    try {
        ...
        const mainOrigin =
            typeof state.mainOrigin === 'string' ? state.mainOrigin : undefined;
        ...
        console.error('[fathom.callback] Failed to decode state:', e);
        return {
            success: false,
...
export default function FathomOAuthCallback() {
