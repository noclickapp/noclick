// PostHog OAuth callback route.
// Handles the redirect from PostHog, decodes the PKCE verifier out of `state`, and
// passes code + verifier back to the opener window via postMessage.
...
export async function loader({
    ...
    if (error) {
        console.error(
            '[posthog.callback] OAuth error:',
            error,
            errorDescription
        );
        return {
            success: false,
            error: errorDescription || `PostHog OAuth error: ${error}`,
        };
    }
    if (!code || !stateB64) {
        console.error('[posthog.callback] Missing code or state');
        return {
            success: false,
    ...
    try {
        ...
        return {
            success: true,
            code,
            redirectUri: process.env.POSTHOG_REDIRECT_URI,
            codeVerifier: state.codeVerifier,
            scopes: state.scopes,
        };
    } catch (e) {
        console.error('[posthog.callback] Failed to decode state:', e);
    ...
    useEffect(() => {
        if (window.opener) {
            window.opener.postMessage(
                { type: 'posthog-oauth-callback', ...data },
