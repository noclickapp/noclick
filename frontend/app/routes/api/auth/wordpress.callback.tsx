// WordPress.com OAuth callback route.
// Handles the redirect from WordPress.com after user grants permission.
...
export async function loader({
    ...
    if (error) {
        console.error(
            '[wordpress.callback] OAuth error:',
            error,
            errorDescription
        );
        ...
        if (error === 'access_denied') {
            return {
                success: false,
                error: 'Access denied. Please grant the required permissions to connect your WordPress.com account.',
            };
        }

        return {
            success: false,
            error: errorDescription || `WordPress OAuth error: ${error}`,
        };
    }

    if (!code || !stateB64) {
        console.error('[wordpress.callback] Missing code or state');
        return {
            success: false,
    ...
    try {
        ...
        if (ageMs > 10 * 60 * 1000) {
            console.error('[wordpress.callback] State expired:', { ageMs });
            return {
                success: false,
        ...
        console.error('[wordpress.callback] Failed to decode state:', e);
        return {
            success: false,
...
export default function WordPressOAuthCallback() {
