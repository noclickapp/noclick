...
// Handles the redirect from Zendesk after the user grants permission.
// Passes the authorization code (and the subdomain from state) back to the
...
export async function loader({
    ...
    if (error) {
        console.error(
            '[zendesk.callback] OAuth error:',
            error,
            errorDescription
        );
        return {
            success: false,
            error: errorDescription || `Zendesk OAuth error: ${error}`,
        };
    }

    if (!code || !stateB64) {
        console.error('[zendesk.callback] Missing code or state');
        return {
            success: false,
    ...
    try {
        ...
        console.error('[zendesk.callback] Failed to decode state:', e);
        return {
            success: false,
...
export default function ZendeskOAuthCallback() {
