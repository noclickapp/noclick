// Attio OAuth callback route.
// Handles the redirect from Attio after user grants permission.
...
export async function loader({
    ...
    if (error) {
        console.error('[attio.callback] OAuth error:', error, errorDescription);
        return {
            success: false,
            error: errorDescription || `Attio OAuth error: ${error}`,
        };
    }

    if (!code || !stateB64) {
        console.error('[attio.callback] Missing code or state');
        return {
            success: false,
    ...
    try {
        ...
        console.error('[attio.callback] Failed to decode state:', e);
        return {
            success: false,
