...
// Handles the redirect from Calendly after the user grants permission.
...
export async function loader({
    ...
    if (error) {
        console.error(
            '[calendly.callback] OAuth error:',
            error,
            errorDescription
        );
        return {
            success: false,
            error: errorDescription || `Calendly OAuth error: ${error}`,
        };
    }
    if (!code || !stateB64) {
        console.error('[calendly.callback] Missing code or state');
        return {
            success: false,
    ...
    try {
        ...
        console.error('[calendly.callback] Failed to decode state:', e);
        return { success: false, error: 'Invalid state parameter' };
    }
}

export default function CalendlyOAuthCallback() {
    ...
    useEffect(() => {
        if (window.opener) {
            window.opener.postMessage(
                { type: 'calendly-oauth-callback', ...data },
