...
// Handles the redirect from Threads after user grants permission.
...
export async function loader({
    ...
    if (error) {
        console.error(
            '[threads.callback] OAuth error:',
            error,
            errorDescription
        );
        return {
            success: false,
            error: errorDescription || `Threads OAuth error: ${error}`,
        };
    }

    if (!code || !stateB64) {
        console.error('[threads.callback] Missing code or state');
        return {
            success: false,
    ...
    try {
        ...
        console.error('[threads.callback] Failed to decode state:', e);
        return {
            success: false,
...
export default function ThreadsOAuthCallback() {
