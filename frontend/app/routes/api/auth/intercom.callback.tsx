// Intercom OAuth callback route.
// Handles the redirect from Intercom after the user grants permission.
...
export async function loader({
    ...
    // If the registered redirect landed this popup on a different origin than
    // the page that opened it (another checkout's port, localhost vs
    // 127.0.0.1), postMessage can never reach the opener — bounce back to the
    // initiating origin recorded in state before completing the handoff.
    if (stateB64) {
        try {
    ...
    if (error) {
        console.error(
            '[intercom.callback] OAuth error:',
            error,
            errorDescription
        );
        return {
            success: false,
            error: errorDescription || `Intercom OAuth error: ${error}`,
        };
    }

    if (!code || !stateB64) {
        console.error('[intercom.callback] Missing code or state');
        return {
            success: false,
    ...
    try {
        ...
        return {
            success: true,
            code,
            redirectUri: process.env.INTERCOM_REDIRECT_URI,
            region: state.region || 'us',
            scopes: state.scopes,
        };
    } catch (e) {
        console.error('[intercom.callback] Failed to decode state:', e);
        return {
            success: false,
...
export default function IntercomOAuthCallback() {
