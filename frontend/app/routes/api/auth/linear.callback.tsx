...
// Handles the redirect from Linear after user grants permission.
...
export async function loader({
    ...
    if (error) {
        console.error(
            '[linear.callback] OAuth error:',
            error,
            errorDescription
        );
        return {
            success: false,
            error: errorDescription || `Linear OAuth error: ${error}`,
        };
    }

    if (!code || !stateB64) {
        console.error('[linear.callback] Missing code or state');
        return {
            success: false,
    ...
    try {
        ...
        console.error('[linear.callback] Failed to decode state:', e);
        return {
            success: false,
...
export default function LinearOAuthCallback() {
