"""
Handler for GitHub OAuth operations.
...
from wss.sender.responses import (
    ...
    GithubOAuthRefreshResponse,
    GithubOAuthValidateResponse,
)
from wss.receiver.client_events import (
    GithubOAuthExchangeRequest,
    GithubOAuthRefreshRequest,
    GithubOAuthValidateRequest,
)

logger = logging.getLogger(__name__)


class GithubOAuthHandler(DatabasePoolMixin, SocketIOHandler):
    """Handler for GitHub OAuth WebSocket events"""
    ...
    def get_events(self) -> Dict[str, Callable]:
        """Register GitHub OAuth events"""
        return {
            "github:oauth:exchange": self.exchange_oauth_code,
            "github:oauth:refresh": self.refresh_oauth_token,
            "github:oauth:validate": self.validate_oauth_token,
        }

    async def setup_user(self, sid: str) -> None:
        _ = sid

    async def exchange_oauth_code(self, sid: str, request: GithubOAuthExchangeRequest) -> None:
        """
        ...
        try:
            ...
            if not user_id:
                await send_event(self.sio, sid, ResponseEvent(
                    ...
                    data=GithubOAuthExchangeResponse(
                        success=False,
            ...
            try:
                ...
                logger.error(f"[GithubOAuthHandler] Token exchange failed: {e}")
                await send_event(self.sio, sid, ResponseEvent(
                    request_id=request.request_id,
                    data=GithubOAuthExchangeResponse(
                        success=False,
            ...
            try:
                ...
                logger.error(f"[GithubOAuthHandler] Encryption failed: {e}")
                await send_event(self.sio, sid, ResponseEvent(
                    request_id=request.request_id,
                    data=GithubOAuthExchangeResponse(
                        success=False,
            ...
            credential_name = user_info.login or user_info.email or f"GitHub ({user_info.id})"
            ...
            async with pool.acquire() as conn:
                ...
                row, error = await create_credential_with_limit_check(
                    conn, user_id, user_tier, 'github_oauth',
                ...
                response = GithubOAuthExchangeResponse(
                    success=True,
                ...
                logger.info(f"[GithubOAuthHandler] Created GitHub credential {row['id']} for user {user_id} ({user_info.login})")

        except Exception as e:
            logger.error(f"[GithubOAuthHandler] Error in exchange_oauth_code: {e}", exc_info=True)
            await send_event(self.sio, sid, ResponseEvent(
                request_id=request.request_id,
                data=GithubOAuthExchangeResponse(
                    success=False,
    ...
    async def refresh_oauth_token(self, sid: str, request: GithubOAuthRefreshRequest) -> None:
        """
        Refresh an expired OAuth token and update the stored credential.
        Note: Only works if the GitHub OAuth App has token expiration enabled.
        """
        try:
            # Get user session
            session = await self.sio.get_session(sid)
            ...
            if not user_id:
                await send_event(self.sio, sid, ResponseEvent(
                    request_id=request.request_id,
                    data=GithubOAuthRefreshResponse(
                        success=False,
                        message="User not authenticated"
                    ).model_dump()
                ))
            ...
            try:
                ...
                logger.error(f"[GithubOAuthHandler] Token refresh failed: {e}")
                await send_event(self.sio, sid, ResponseEvent(
                    request_id=request.request_id,
                    data=GithubOAuthRefreshResponse(
                        success=False,
            ...
            response = GithubOAuthRefreshResponse(
                success=True,
            ...
            logger.info(f"[GithubOAuthHandler] Refreshed token for credential {request.credential_id}")

        except Exception as e:
            logger.error(f"[GithubOAuthHandler] Error in refresh_oauth_token: {e}", exc_info=True)
            await send_event(self.sio, sid, ResponseEvent(
                request_id=request.request_id,
                data=GithubOAuthRefreshResponse(
                    success=False,
    ...
    async def validate_oauth_token(self, sid: str, request: GithubOAuthValidateRequest) -> None:
        """
        Validate if a stored OAuth credential is still valid.
        Note: GitHub tokens don't expire by default, so this checks if refresh token exists.
        """
        try:
            # Get user session
            session = await self.sio.get_session(sid)
            ...
            if not user_id:
                await send_event(self.sio, sid, ResponseEvent(
                    request_id=request.request_id,
                    data=GithubOAuthValidateResponse(
                        valid=False,
                        message="User not authenticated"
                    ).model_dump()
                ))
            ...
            async with pool.acquire() as conn:
                ...
                try:
                    ...
                    logger.error(f"[GithubOAuthHandler] Decryption failed: {e}")
                    await send_event(self.sio, sid, ResponseEvent(
                        request_id=request.request_id,
                        data=GithubOAuthValidateResponse(
                            valid=False,
                ...
                expires_at = credential_data.get('expires_at')
                login = credential_data.get('login') or row['metadata'].get('login')
                ...
                if not expires_at:
                    ...
                    response = GithubOAuthValidateResponse(
                        valid=True,
                        expires_soon=False,
                        login=login,
                        email=email,
            ...
            logger.error(f"[GithubOAuthHandler] Error in validate_oauth_token: {e}", exc_info=True)
            await send_event(self.sio, sid, ResponseEvent(
                request_id=request.request_id,
                data=GithubOAuthValidateResponse(
                    valid=False,
