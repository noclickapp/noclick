...
def test_brain_model_follows_the_available_key(monkeypatch):
    assert builder_config.resolve_brain_model() == "openrouter/openai/gpt-6-luna", (
        "with no key at all the OpenRouter route is named — the key the builder asks for"
    )
    monkeypatch.setenv("OPENAI_API_KEY", "sk")
    assert builder_config.resolve_brain_model() == "openai/gpt-6-luna"
...
def test_missing_brain_key_names_the_variable_and_provider(monkeypatch):
    assert builder_config.missing_brain_key("openrouter/openai/gpt-6-luna") == ("OPENROUTER_API_KEY", "openrouter")
    monkeypatch.setenv("OPENROUTER_API_KEY", "sk-or")
    assert builder_config.missing_brain_key("openrouter/openai/gpt-6-luna") is None
