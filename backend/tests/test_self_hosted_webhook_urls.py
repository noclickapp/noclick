...
def test_the_delivery_route_prefix_is_part_of_the_minted_url(monkeypatch):
    ...
    assert webhook_delivery.get_webhook_url("abc-123") == "https://noclick.example.com/webhook/abc-123"
...
def test_explicit_webhook_base_wins_over_the_api_url(monkeypatch):
    ...
    assert webhook_delivery.get_webhook_url("x") == "https://hooks.example.com/webhook/x"


def test_no_base_fails_loudly():
    with pytest.raises(RuntimeError):
        webhook_delivery.get_webhook_url("x")
